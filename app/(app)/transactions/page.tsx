...
function TransactionsInner() {
    ...
    const [form, setForm] = useState({ customerId: preCustomerId, amount: '', date: new Date().toISOString().split('T')[0], note: '', paymentMode: '' })
    ...
    function openCreate() { setEditTxn(null); setForm({ customerId: filterCust || preCustomerId, amount: '', date: new Date().toISOString().split('T')[0], note: '', paymentMode: tab === 'COLLECTION' ? 'CASH' : '' }); setSearchQuery(''); setError(''); setModal(true) }
    function openEdit(t: TxnRow) { setEditTxn(t); setForm({ customerId: t.customer.customerNumber, amount: String(t.amount), date: t.date.split('T')[0], note: t.note ?? '', paymentMode: (t as any).paymentMode ?? '' }); setError(''); setModal(true) }

    async function handleSave() {
        setError(''); setSaving(true)
        try {
            if (editTxn) {
                const res = await fetch(`/api/transactions/${editTxn.id}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ amount: form.amount, date: form.date, note: form.note, paymentMode: form.paymentMode }) })
                        ...
                        <div key={t.id} className="card" style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                            <div style={{ flex: 1, minWidth: 0 }}>
                                ...
                                <div className="text-muted text-xs">
                                    ...
                                    {(t as any).paymentMode && ` · ${(t as any).paymentMode}`}
                                    {` · ${t.createdBy.name}`}
                                    {t.note && ` · ${t.note}`}
                                </div>
                            </div>
                ...
                <div className="modal-overlay" onClick={e => e.target === e.currentTarget && setModal(false)}>
                    <div className="modal">
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                                ...
                                <div className="form-group">
                                    ...
                                    >
                                        ...
                                        {customers
                                            ...
                                            .map(c => <option key={c.id} value={c.id}>{c.customerNumber} – {c.name}</option>)
                                        }
                                    </select>
                                </div>
                            )}
                            ...
                            <div className="form-grid form-grid-2">
                                <div className="form-group">
                                    <label className="form-label">{tab === 'COLLECTION' ? 'Payment Mode' : 'Remarks'}</label>
                                    {tab === 'COLLECTION' ? (
                                        ...
                                        <input className="form-input" placeholder="Optional remarks" value={form.note} onChange={e => setForm(f => ({ ...f, note: e.target.value }))} />
                                    )}
                                </div>
                                {tab === 'COLLECTION' && (
                                    <div className="form-group">
