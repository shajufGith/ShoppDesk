...
function ReportsInner() {
    ...
    async function runReport() {
        ...
        const params = new URLSearchParams({ type, ...(dateRange.from ? { from: dateRange.from } : {}), ...(dateRange.to ? { to: dateRange.to } : {}), ...(custId ? { customerId: custId } : {}), ...(segId ? { segmentId: segId } : {}) })
    ...
    return (
        <div className="page-content">
            ...
            <button id="run-report-btn" className="btn btn-primary btn-full mb-2" onClick={runReport} disabled={loading}>
            ...
            {data && (type === 'sales' || type === 'collection') && (
                <>
                    ...
                    <div className="table-wrap">
                        <table>
                            <thead><tr><th>Date</th><th>Customer</th><th>Segment</th><th>Amount</th><th>Note</th><th>By</th></tr></thead>
                            <tbody>
                                {data.rows.map((r: any) => (
                                    <tr key={r.id}>
                                        <td>{fmtDate(r.date)}</td>
                                        <td>{r.customer.name} <span className="text-muted text-xs">#{r.customer.customerNumber}</span></td>
                                        <td>{r.customer.segment?.name ?? '—'}</td>
            ...
            {data && type === 'due' && (
                <>
                    ...
                    <div className="table-wrap">
                        <table>
                            <thead><tr><th>#</th><th>Customer</th><th>Segment</th><th>Amount Due</th></tr></thead>
                            <tbody>
                                {data.rows.map((r: any) => (
                                    <tr key={r.id}>
